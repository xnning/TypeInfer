...
$\Gamma, \Theta, \Delta  = \cdot~|~\Gamma, x~|~\Gamma, x:\sigma~|~\Gamma, \genA~|~\Gamma, \genA=\tau~|~\Gamma, \genA:\tau~|~\Gamma, \genA:\tau = \tau_2~|~\Gamma, \blacktriangleright_{\genA}$
...
{
\Gamma[\genA_1, \genA_2, \genA = \Pi x:\genA_1. \genA_2] \checktypeno e_2 : \genA_1 \dashv \Theta
}
{\Gamma[\genA]\infertype^{app} \genA \cdot e_2 : \genA_2 \dashv \Theta}
$}
\]
...
\Theta_1[\genA_1, \genA_2, \genA = \Pi x:\genA_1. \genA_2] \checktypeno e_2 : \genA_1 \dashv \Theta
}
{\Gamma[\genA:\tau_\alpha]\infertype^{app} \genA \cdot e_2 : \genA_2 \dashv \Theta}
$}
\]
...
\framebox{$\Gamma[\alpha] \dskunify \genA \sqsubseteq \rho \dashv \Theta, \alpha \notin ftv(\rho)$}
...
{ }
{\Gamma[\genA\{:\tau_\alpha\} ] \dskunify \genA \sqsubseteq \genA \dashv \Gamma[\genA\{:\tau_\alpha\}] }
\]

\[
\inferrule*[right=EVar-Uni]
{ \Gamma_1, \genA, extract\_evar(\Gamma_2) \vdash \tau_1 \\
\Gamma_1, \genA, \Gamma_2 \co \tau_1 \bemono \tau_2 \dashv \Theta[\genA]
}
{\Gamma_1, \genA, \Gamma_2 \unify \genA \sqsubseteq \tau_1 \dashv \Theta[\genA=\tau]}
\]

\[
\inferrule*[right=EVar-Dsk]
{\Gamma_1, \genA, extract\_evar(\Gamma_2) \vdash \sigma \\
\Gamma_1, \genA, \Gamma_2 \co \sigma \bemono \tau \dashv \Theta[\genA] }
{ \Gamma_1, \genA, \Gamma_2 \dsk \genA \sqsubseteq \sigma \dashv \Theta[\genA = \tau] }
\]

the difference between evar-uni and evar-dsk is that evar-uni requests the rhs is a mono type.
\newline

\framebox{$\Gamma[\alpha:\tau_\alpha] \dskunify \genA \sqsubseteq \rho \dashv \Theta, \alpha \notin ftv(\rho)$}

\[
\inferrule*[right=EVar-Typed]
{
\sigma_\rho =  get\_type(\rho)\\
\Gamma_{\genA} \dsk[\Gamma_{\genA}] \sigma_\rho \sqsubseteq [ \Gamma_{\genA} ]\tau_\alpha \dashv \Theta_1[\genA :\tau_1 \{=\tau_2\}]\\
\Theta_1[\genA\{=\tau_2\}] \dskunify [\Theta_1]\genA \sqsubseteq [\Theta_1]\rho \dashv \Theta
}
{\Gamma[\genA:\tau_\alpha] \dskunify \genA \sqsubseteq \rho \dashv \Theta }
\]

this rule first confirm the type constraint is satisfied. Then throw the type constraint to continue the process.
\newline


\framebox{$\Gamma[\alpha] \cocontra \sigma \bemono \tau \dashv \Theta$} $\phi = co~|~contra$
...
{ }
{\Gamma[\genB\{:\tau\}][\genA] \cocontra \genB \bemono \genB \dashv \Gamma[\genB\{:\tau\}][\genA]}
\]

\[
\inferrule*[right=Mono-EVar-right]
{ }
{\Gamma[\genA][\genB\{:\tau\}] \cocontra \genB \bemono \genB_1 \dashv \Gamma[\genB_1\{:\tau\}, \genA][\genB = \genB_1]}
\]
