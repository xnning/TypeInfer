...
\rul{T-Lam$\Inf$} shows one difference between our system and Dunfield's system which is due to dependent types. Consider the rule for lambda in Dunfield's system:
...
The interesting cases come from the rule \rul{U-EVarTy} and \rul{U-TyEVar}. They behave the same no matter the existential variable appears in which side. So take \rul{U-EVarTy} as example. In Dunfield's system, we have rule:
