...
Hole notation is also used for replace and modification. Later in the unification rules, we will see situations where $\tctx[\genA]$ could become $\tctx[\genA = \star]$, which means the context keeps unchanged except $\genA$ now is solved.
...
Figure \ref{fig:algo-typing-rules} gives the algorithmic bidirectional typing rules.
