...
\newcommand*{\TLamInf}{\inferrule{\preinf[,\genA,x:\genA]
    ...
    \tau_2 \toctx \trto {\blam x \genA t}}\rname{T-Lam$\Inf$}}
...
\newcommand*{\UEVarTy}{\inferrule{\genA \not \in \fun{FV}(\tau) \\ \ctx[\genA], \emptyset \bycg \tau \cgto \tau' \toctx[\genA]}{\ctx[\genA] \byuni \genA \lt \tau \toctx[\genA=\tau']}\rname{U-EvarTy}}
\newcommand*{\UTyEVar}{\inferrule{\genA \not \in \fun{FV}(\tau) \\ \ctx[\genA], \emptyset \bycg \tau \cgto \tau' \toctx[\genA]}{\ctx[\genA] \byuni \tau \lt \genA \toctx[\genA=\tau']}\rname{TyEVar}}
...
\newcommand*{\CVarA}{\inferrule{ }{\ctx[x][\genA], \varset \bycg x \cgto x \toctxo[x][\genA]}\rname{C-Var1}}
\newcommand*{\CVarB}{\inferrule{ }{\ctx, \varset[x] \bycg x \cgto x \toctxo}\rname{C-Var2}}
\newcommand*{\CStar}{\inferrule{ }{\ctx, \varset \bycg \star \cgto \star \toctxo}\rname{C-Star}}
\newcommand*{\CEVarA}{\inferrule{ }{\ctx[\genB][\genA], \varset \bycg \genB \cgto \genB \toctxo[\genB][\genA]}\rname{C-EVar1}}
\newcommand*{\CEVarB}{\inferrule{ }{\ctx[\genA][\genB], \varset \bycg \genB \cgto \genA_1 \toctxo[\genA_1, \genA][\genB=\genA_1]}\rname{C-EVar2}}
\newcommand*{\CPi}{\inferrule{\ctx, \varset \bycg \tau_1 \cgto \tau_3 \toctx_1 \\ \ctxl_1 , x:\varset \bycg [\ctxl_1]\tau_2 \cgto \tau_4 \toctx}{\ctx, \varset \bycg \bpi x {\tau_1} {\tau_2} \cgto \bpi x {\tau_3} {\tau_4} \toctx}\rname{C-Pi}}
\newcommand*{\CLam}{\inferrule{\ctx, x: \varset \bycg \tau_1 \cgto \tau_2 \toctx}{\ctx, \varset \bycg \erlam x {\tau_1} \cgto \erlam x {\tau_2} \toctx}\rname{C-Lam}}
\newcommand*{\CLamAnn}{\inferrule{\ctx, \varset \bycg \tau_1 \cgto \tau_3 \toctx_1 \\ \ctxl_1, x:\varset \bycg [\ctxl_1]\tau_2 \cgto \tau_4 \toctx }{\ctx, \varset \bycg \blam x {\tau_1} {\tau_2} \cgto \blam x {\tau_3} {\tau_4} \toctx}\rname{C-LamAnn}}
\newcommand*{\CLet}{\inferrule{\ctx, \varset \bycg \tau_1 \cgto \tau_3 \toctx_1 \\ \ctxl_1, x:\varset \bycg [\ctxl_1]\tau_2 \cgto \tau_4 \toctx }{\ctx, \varset \bycg \kw{let} x = \tau_1 \kw{in} \tau_2 \cgto \kw{let} x = \tau_3 \kw{in} \tau_4 \toctx}\rname{C-Let}}
\newcommand*{\COthers}{\inferrule{\ctx, \varset \bycg \tau_0 \cgto \tau_0' \toctx_1 \\ \ctxl_i, \varset \bycg [\ctxl_i]\tau_i \cgto \tau_i' \toctx_{i+1}}{\ctx, \varset \bycg T\ \overbar{\tau_n} \cgto T\ \overbar{\tau_n'}}\rname{C-Other}}
...
\headercapm{\ctx[\genA], \varset \bycg \tau_1 \cgto \tau_2 \toctx}{Unify TVar and Expression}
