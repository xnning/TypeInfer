...
and even harder to reason about. \bruno{Say more as to why beta-equality is a
  main complication?} \ningning{How about now?} However beta-equality is not the
...
\item \textbf{We show that type sanitization works for dependent types}. This is
  ...
  dependent types.~\bruno{true? How about Dunfield's follow up paper?}
  \ningning{ if you mean ``Sound and Complete Bidirectional Typechecking for
    HigherRank Polymorphism with Existentials and Indexed Types'', no, i don't
    think their approach can deal with dependent types. I cited the paper here.
  }
